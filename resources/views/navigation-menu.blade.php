                            ...
                            <x-slot name="trigger">
                                @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                                    <button aria-label="Configurações da Conta"
                                        ...
                                        <x-image class="h-12 w-12 rounded-full object-cover" :src="Auth::user()->profile_photo" alt="{{ Auth::user()->name }}" />
                                    @else
                                        <!-- Exibe as iniciais se não houver foto de perfil -->
                                        <x-initials-avatar :user="Auth::user()" class="h-12 w-12" />
                                    @endif


                                    </button>
                                @else
                                    <!-- Caso não gerencie fotos de perfil, exibe o nome do usuário -->
